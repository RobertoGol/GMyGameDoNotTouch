...
- `Admin / Moderator Launcher`
  - один launcher class/product для admin и moderator;
  - текущий operations-deck / terminal-style launcher сохраняется для этого use-case;
  - может показывать:
    - route state;
    - profile/world/session state;
    - LAN/session status;
    - recovery/industry/backbone state;
    - editor launch;
    - debug/admin readouts.
- future `Player Launcher`
  - отдельный player-facing launcher для обычных игроков;
  - atmospheric world-entry UI;
  - tonal/UI-IA опора на `Log Horizon` / `Elder Tale`;
...
- `Pip-Boy 3000 Mark V`
  - heavy failed model;
  - uses crude steel / heavy metal вместо practical light alloy;
  - discarded в wire trash bin среди papers/workshop waste;
  - rejected/unwanted;
  - не входит в normal selectable set.
- canonical top tabs: `STAT`, `ITEM`, `DATA`, `MAP`, `RADIO`;
- `DATA` работает как live activity journal: `MAIN`, `SIDE`, `DAILY`, `EVENTS`, `OPS`, `ARCHIVE`;
- `Fallout 4/76` использовать только как:
...
- `BT-72` second-seat policy теперь не расходится с runtime loop: локальный seat toggle уважает `pilot_only / trusted_only / open_crew`, отдает читаемый deny/success feedback и покрыт smoke-checks;
